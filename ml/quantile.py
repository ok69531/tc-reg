...
def main():
    ...
    params_dict = {
        "alpha": [1e-6, 5e-6, 1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 5e-3, 1e-2, 5e-2, 1e-1, 5e-1, 1.0],
    ...
    for p in tqdm(range(len(params))):
        ...
        save_results(result, path = save_path, file_name = f'quantile_{args.fp_type}.pkl')
        # save_results(result, path = save_path, file_name = f'quantile_{args.fp_type}_feat_sel_{args.use_feat_sel}.pkl')
    ...
    file_name = f'best_quantile_{args.fp_type}.pkl'
